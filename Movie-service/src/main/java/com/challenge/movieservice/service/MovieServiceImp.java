...
import com.challenge.movieservice.proxy.RecommendationProxy;
...
public class MovieServiceImp implements MovieService{
    ...
    private RecommendationProxy recommendationProxy;
    @Override
    ...
    @Override
    public List<Long> ShowMoviesWithoutPageable() {
        return movieRepository.getAllIds();
    }

    @Override
    public List<MovieDTO> getRecommendedMovies(int size) {
        List<MovieDTO> moviesRecommended = new ArrayList<>();
        recommendationProxy.getRecommendations(size).forEach(
                recommendedMoviesDTO -> {
                    moviesRecommended.add(getMovie(recommendedMoviesDTO.getIdMovie()));
                }
        );
